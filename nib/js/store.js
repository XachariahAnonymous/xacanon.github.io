...
(function (NIB) {
  ...
  const Local = (() => {
    ...
    function load() {
      ...
      const base = { loggedIn: false, uid: null, email: null, displayName: null, walletAddress: null,
        balance: 100, minted: {}, collection: [], openings: [], config: DEFAULT_CONFIG(), admins: [] };
  ...
  const Fire = (() => {
    ...
    async function buyAndOpenPack() {
      ...
      // One atomic batch: cap-guarded card increments + collection + debit
      // + audit + cosmetic stats. Rules validate each write.
      const uid = cache.uid;
      ...
      catch (e) { return { ok: false, error: "Pack open failed (try again). " + (e.message || "") }; }
