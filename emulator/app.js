...
(() => {
  ...
  const EXT_TO_CORE = {
    ...
    // .chd, .iso and .bin are NOT mapped here on purpose: they're shared by
  ...
  const AMBIGUOUS_DISC_EXTS = ["chd", "iso", "bin"];
  const DISC_SYSTEMS = [
    { core: "psx", label: "PS1" },
    { core: "ps2", label: "PS2" },
  ...
  async function launchGame(id) {
    ...
    // PS2 doesn't go through EmulatorJS: it's Play! (jpd002/Play-), a
    // separately built Emscripten/WASM app hosted alongside this site.
    // It has no external URL/blob loading API, so instead of a srcdoc page
    // we point the iframe straight at its build and hand the game over via
    // the same iframe-ready/load-rom postMessage handshake used below, but
    // passing the stored Blob/File directly (structured-cloned, not copied)
    // since PS2 images are multi-gigabyte and Play! reads it lazily via
    // slice() rather than needing the whole thing buffered up front.
    if (rom.core === "ps2") {
      iframe.src = "ps2-player/index.html";
      const messageHandler = (e) => {
        if (e.source !== iframe.contentWindow) return;
        if (!e.data || e.data.type !== "iframe-ready") return;
        window.removeEventListener("message", messageHandler);
        iframe.contentWindow.postMessage(
          { type: "load-rom", file: rom.blob, name: rom.filename },
          "*"
        );
      };
